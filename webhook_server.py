...
processed_alerts = {}
# 告警记录文件路径
ALERT_RECORD_FILE = '/tmp/processed_alerts.json'
# 文件锁
    ...
    try:
        if os.path.exists(ALERT_RECORD_FILE):
            with open(ALERT_RECORD_FILE, 'r') as f:
                ...
                logger.info(f"已加载 {len(processed_alerts)} 条告警处理记录")
    except Exception as e:
        logger.error(f"加载告警处理记录失败: {str(e)}")
    ...
    try:
        with file_lock:
            ...
            with open(ALERT_RECORD_FILE, 'w') as f:
                json.dump(data, f)
            logger.info(f"已保存 {len(processed_alerts)} 条告警处理记录")
    except Exception as e:
        logger.error(f"保存告警处理记录失败: {str(e)}")
    ...
    try:
        ...
        if alert_key in processed_alerts:
            ...
            if current_time - last_processed < timedelta(hours=1):
                logger.info(f"告警 {alert_key} 在1小时内已处理过，跳过处理")
                return True
            else:
                logger.info(f"告警 {alert_key} 上次处理时间已超过1小时，允许重新处理")
        else:
            logger.info(f"告警 {alert_key} 首次处理")
    except Exception as e:
        logger.error(f"检查告警时间时发生错误: {str(e)}")
        return False
...
def mark_alert_processed(alert):
    ...
    if alert_key:
        ...
        logger.info(f"标记告警 {alert_key} 为已处理，时间: {processed_alerts[alert_key]}")
        # 保存到文件
...
def webhook():
    try:
        data = request.json
        logger.info(f"收到告警数据: {json.dumps(data, ensure_ascii=False)}")
        ...
        if isinstance(data, list):
            ...
            logger.warning(f"未知的告警数据格式: {type(data)}")
        ...
        for alert in alerts:
            ...
            if alert.get('status') != 'firing':
                logger.info(f"跳过非firing状态的告警: {get_alert_key(alert)}")
                continue
                
            if alert.get('labels', {}).get('alertname') == 'HeapMemoryHighUsage':
                instance = alert['labels'].get('instance')
                logger.info(f"检测到堆内存告警: {instance}")
                
                # 检查是否已经处理过这个告警
                if is_alert_processed(alert):
                    logger.info(f"跳过已处理的告警: {get_alert_key(alert)}")
                    continue
                
                # 获取容器名称
                container = get_container_name(instance)
                if not container:
                    logger.warning(f"无法从实例名称 {instance} 中提取容器名称，跳过处理")
                    continue
                    
                namespace = 'hems-prd'  # 根据您的配置设置
                
                logger.info(f"开始处理告警，实例: {instance}, 命名空间: {namespace}, 容器: {container}")
                ...
                if result.stdout:
                    logger.info(f"堆转储执行结果: {result.stdout}")
                if result.stderr:
                    logger.error(f"堆转储执行错误: {result.stderr}")
                else:
                    # 只有在成功执行堆转储后才标记为已处理
                    mark_alert_processed(alert)
                    logger.info(f"告警处理完成: {get_alert_key(alert)}")
